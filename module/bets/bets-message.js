...
const { addSettleBet, insertBets } = require('./bets-db');
...
const placeBet = async (io, socket, betData) => {
    ...
    const betObj = { bet_id, token, socket_id: parsedPlayerDetails.socketId, game_id: playerDetails.game_id, roomId };
    ...
    userBets.forEach(bet => {
        ...
        if (!roomDetails) return logEventAndEmitResponse(socket, data, 'Invalid Room Id Passed', 'bet');
        if (!roomDetails.chips.includes(Number(betAmount))) return logEventAndEmitResponse(socket, data, 'Invalid Bet Amount', 'bet');
        ...
        if (Number(betAmount) < roomDetails.min || Number(betAmount) > roomDetails.max) {
    ...
    if (totalBetAmount > 0) {
        ...
        try {
            ...
            return socket.emit('message', `Bet cancelled by upstream`);
        }
        ...
        return io.emit('message', { eventName: "bet", data: { message: "Bet Placed successfully" } });
    }
}

const settleBet = async (io, winningNumber) => {
    try {
        let oddsData = {
        ...
        if (cachedBets) {
            ...
            await Promise.all(bets.map(async betData => {
                ...
                if (finalAmount > 0) {
                    ...
                    io.to(socket_id).emit('message', { eventName: 'settlement', data: { message: `You won ${winAmount}` } });
                }
            }));
            Object.assign(oddsData, { totalBetAmount: oddsData.totalBetAmount + sessionBetAmount, winCount: oddsData.winCount + sessionWinCount});
